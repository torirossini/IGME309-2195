...
//Allows forward and backwards movement of the camera
...
void MyCamera::MoveVertical(float a_fDistance)
{
	...
	m_v3Above += upVector * a_fDistance;
}

//Allows movement to the left and right of the camera
...
//Handle rotation
void MyCamera::RotateView(float xAngle, float yAngle, float multiplier)
{
	//There's a multiplier here so it isn't so slow
	//yaw angle
	quaternion m_xAngleAxis = glm::angleAxis(glm::radians(yAngle) * multiplier, upVector);

	//pitch angle
	quaternion m_yAngleAxis = glm::angleAxis(glm::radians(xAngle) * multiplier, -rightVector);
	quaternion m_rotation;

	//Calculate the total rotation on each axis and apply that rotation to the target view
	m_rotation = m_xAngleAxis * m_yAngleAxis;
	m_v3Target = m_v3Position + ((m_v3Target - m_v3Position) * m_rotation);

	// reset the camera vectors
	forwardVector = glm::normalize(m_v3Target - m_v3Position);
	rightVector = glm::normalize(glm::cross(forwardVector, glm::normalize(m_v3Above - m_v3Position)));
	upVector = glm::normalize(m_v3Above - m_v3Position);
