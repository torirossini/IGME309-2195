...
class MyCamera
{
	...
	vector3 rightVector = glm::normalize(glm::cross(m_v3Above, forwardVector));
	vector3 upVector = glm::cross(forwardVector, rightVector);
...
public:
	...
	*/
	void RotateView(float xAngle, float yAngle, float multipler = 1.0f);
};
